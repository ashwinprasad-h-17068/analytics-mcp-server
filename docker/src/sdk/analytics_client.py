...
class AnalyticsClient:
    ...
    class WorkspaceAPI:
        ...
        def get_aggregate_formulas(self, config = {}):
            """
        ...
        def get_aggregate_formula_dependents(self, formula_id):
            """
            Returns list of all dependent views and formulas for the specified aggregate formula.
            @param formula_id: Id of the aggregate formula
            ...
            @rtype:dictionary
            """
            endpoint = self.endpoint + "/aggregateformulas/" + formula_id + "/dependents"
            response = self.ac.send_api_request("GET", endpoint, None, self.request_headers)
            return response["data"]

        def get_aggregate_formula_value(self, formula_id):
            """
            ...
            @raise ServerError: If the server has received the request but did not process the request due to some error.
            @raise ParseError: If the server has responded but client was not able to parse the response.
            @return: Aggregate formula value.
            @rtype:string
            """
            endpoint = self.endpoint + "/aggregateformulas/" + formula_id + "/value"
            response = self.ac.send_api_request("GET", endpoint, None, self.request_headers)
            return response["data"]["formulaValue"]
        ...
        def create_report(self, config = {}):
            ...
            @raise ServerError: If the server has received the request but did not process the request due to some error.
            @raise ParseError: If the server has responded but client was not able to parse the response.
            @return: Id of the created view.
            @rtype:string
            """
            endpoint = self.endpoint + "/reports"
            response = self.ac.send_api_request("POST", endpoint, config, self.request_headers)
            return int(response["data"]["viewId"])

        def update_report(self, view_id, config = {}):
            """
            ...
            @raise ParseError: If the server has responded but client was not able to parse the response.
            """
            endpoint = self.endpoint + "/reports/" + view_id
            response = self.ac.send_api_request("PUT", endpoint, config, self.request_headers)
    ...
    class ViewAPI:
        ...
        def get_formula_columns(self):
            ...
            @rtype:list
            """
            endpoint = self.endpoint + "/formulacolumns"
            response = self.ac.send_api_request("GET", endpoint, None, self.request_headers)
            return response["data"]["formulaColumns"]
        ...
        def edit_formula_column(self, formula_id, expression, config = {}):
            ...
            @raise ParseError: If the server has responded but client was not able to parse the response.
            """
            endpoint = self.endpoint + "/formulacolumns/" + formula_id
            config["expression"] = expression;
            self.ac.send_api_request("PUT", endpoint, config, self.request_headers)
    ...
    def send_batch_import_api_request(self, request_url, config, request_headers, file_path, batch_size, tool_config):
        if not self.auth.is_remote and self.auth.get_access_token() is None:
        ...
        for i in range(total_batch_count):
            ...
            resp_obj = self.submit_import_request(request_url, config_data, request_headers, self.auth.get_access_token(), files)

            if not (str(resp_obj.status_code).startswith("2")):
                if not self.auth.is_remote and self.is_oauth_expired(resp_obj):
                    self.regenerate_analytics_oauth_token()
                    resp_obj = self.submit_import_request(request_url, config_data, request_headers, self.auth.get_access_token(),
                                                          files)
                    if not (str(resp_obj.status_code).startswith("2")):
    ...
    def send_import_api_request(self, request_url, config, request_headers, file_path, data=None):
        ...
        """
        if not self.auth.is_remote and self.auth.get_access_token() == None:
            self.regenerate_analytics_oauth_token()
    ...
    def send_export_api_request(self, request_url, config, request_headers, file_path):
        ...
        resp_obj = self.submit_export_request(request_url, config_data, request_headers, self.auth.get_access_token())
    ...
    def send_api_request(self, request_method, request_url, config, request_headers, is_json_response = True):
        ...
        resp_obj = self.submit_request(request_method, request_url, config_data, request_headers, self.auth.get_access_token())
    ...
    def regenerate_analytics_oauth_token(self):
        ...
        """
        oauth_params = {}
        oauth_params["client_id"] = self.auth.client_id
        oauth_params["client_secret"] = self.auth.client_secret
        oauth_params["refresh_token"] = self.auth.refresh_token
