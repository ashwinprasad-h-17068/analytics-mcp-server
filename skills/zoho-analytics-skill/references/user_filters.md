...
## Using User Filters with updateReport

**Critical:** `updateReport` performs a **full replacement**. If you omit `userFilters`, all existing user filters are cleared.

**Always follow this workflow:**
1. Call `readReportMetadata` to retrieve current configuration including existing `userFilters`
2. Modify the `userFilters` array as needed (add/remove/update)
3. Re-submit the complete `userFilters` array via `updateReport`

See [Read & Update Reports](./reports_read_update.md) for the full update workflow.

### Example — Add a User Filter to Existing Report
...
**Step 2 — Inspect returned JSON, note existing userFilters (if any)**

**Step 3 — Update with modified userFilters array:**
```
execute_analytics_tool(
    "updateReport",
    {
...
6. **Test filter values**: Before creating user filters, query the data to understand available values
7. **Read before update**: Always call `readReportMetadata` before `updateReport` to preserve existing configuration
